  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text)]">Token Activity</h1>
        ...
        <div className="flex items-center gap-3">
          <div className="flex rounded-xl border border-[var(--border)] overflow-hidden">
            {statuses.map(v => (
              <button key={v} onClick={() => setStatusFilter(v)}
                className={`px-3 py-1.5 text-xs font-medium capitalize transition-all ${statusFilter === v ? 'bg-brand-500 text-[var(--text)]' : 'text-slate-400 hover:text-[var(--text)]'}`}>
                {v}
              </button>
            ))}
          </div>
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 rounded-xl bg-[var(--hover)] border border-[var(--border)] text-sm text-[var(--text)] placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-brand-500/30 w-56"
            />
          </div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {filtered.map(r => (
                  <tr key={r.id} className="hover:bg-[var(--hover)] transition-colors">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <p className="font-medium text-[var(--text)]">{r.user_name}</p>
                    ...
                    <td className="px-4 py-3 font-mono font-semibold text-[var(--text)]">{r.tokens_used.toLocaleString('en-IN')}</td>
